...
// Are you a UK national? (if RETIRED and living in the EU)
router.post('/euBornInUk', function (req, res) {
  ...
  if (euBornInUk == "Yes"){
    ...
    res.redirect('settled-status-s1')
  }
})
...
// Are you a UK national? (if STUDYING and living in the EU)
...
// Are you a UK national? (if WORKING and living in the EU)
router.post('/euWorkingBornInUk', function (req, res) {
  var euWorkingBornInUk = req.session.data['posted-born-in-uk']
  if (euWorkingBornInUk == "Yes"){
    res.redirect('application-pw/evidence-pw')
  }
  else {
    res.redirect('settled-status-pw')
  }
})
...
// Posted worker form type - with Settled Status
router.post('/application-pw-ss/pwFormType', function (req, res) {
  var pwFormType = req.session.data['form-type']
  if (pwFormType == "A1"){
    res.redirect('a1-country')
  }
